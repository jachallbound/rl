...
blackjack_action hand_dealer_plays(hand* dealer) {
  ...
  do {
    if (dealer->value < 17) {
      action = HIT;
      hand_add_card(dealer);
      if (++hits > 100) exit(1); /* infinite loop, error out */
    } else {
      action = STICK;
    }
    hand_calculate_value(dealer);
...
int hand_has_usable_ace(hand* h) {
  ...
  for (int c = 0; c < h->L; c++)
    if (h->cards[c].n == 1) return 1;
  return 0;
}

int hand_value_before_busting(hand* h) {
  if (h->bust) { /* hand busted, get value when hitting busted */
    return (h->value - h->cards[h->L-1].n);
  } else { /* hand did not bust, get full hand value */
    return (h->value);
  }
}
